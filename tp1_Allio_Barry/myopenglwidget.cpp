...
void myOpenGLWidget::makeGLObjects()
{
    ...
    ListePoint = Discretisation::discret_segments(ListeSegment, 4.f);

    _nbVertex = ListePoint.size();
    _dimVertex = 3;
    _drawMode = GL_LINES;

    QVector<GLfloat> vertData;
    vertData = PointToVbo::convert_listPoints_to_vbo(ListePoint, colors);
...
void myOpenGLWidget::paintGL()
{
    ...
    m_program->setAttributeBuffer("posAttr",
                                  GL_FLOAT, 0 * sizeof(GLfloat), _dimVertex, taille * sizeof(GLfloat));
    ...
    for (unsigned i=0; i<_listSegment.size(); i++)
    {
        int pointBegin;
        int pointEnd; */
        glDrawArrays(GL_LINE_STRIP, 0, _nbVertex);
        glDrawArrays(GL_POINTS, 0, _nbVertex);
    //}
