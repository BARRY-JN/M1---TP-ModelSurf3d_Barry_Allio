...
std::vector<Point> tab_to_list(vector<vector<Point>> tab, int n, int m)
{
...
void myOpenGLWidget::makeGLObjects()
{
    ...
    m_vbo3.create();
    */
    CarBez = new CarreauxBezier(n, m);
    CarBez->display_points();

    vector<Point> pointControles = tab_to_list(CarBez->coords(), n, m);
    vertData1 = PointToVbo::convert_to_grid(pointControles, n, m, 100, 0, 0);
...
void myOpenGLWidget::update_discret_carBez()
{
    CarBez->generer_carreaux(0, NbU, NbV);
    vector<Point> coordsCarreaux = tab_to_list( CarBez->coordsCarreaux(), NbU, NbV );

    if (Triangle){
        vertData2 = PointToVbo::convert_triangulation(coordsCarreaux, NbU, NbV, 0.f, 100.f, 0.f);
    }else {
        vertData2 = PointToVbo::convert_to_grid(coordsCarreaux, NbU, NbV,0.f, 100.f, 0.f);
    }
...
void myOpenGLWidget::paint_vbo(int idVbo, QMatrix4x4 *mat)
{
    ...
    if (idVbo==1)
    {
        if (Vertices){glDrawArrays(GL_POINTS, 0, n*m);}
        if (!Edges) return;
...
//Effectue carreaux par triangulation
void myOpenGLWidget::carreaux_tri()
{
    if (Vertices){
        glDrawArrays(GL_POINTS, 0, (NbU-1)*(NbV-1)*6);
    }
    if (!Edges) return;
    for (int i=0; i<(NbU-1)*(NbV-1)*6; i+=3){
        if (Forme){
...
//Effectue carreaux par lignes
void myOpenGLWidget::carreaux_lines()
{
    if (Vertices){
        glDrawArrays(GL_POINTS, 0, NbU*NbV);
    }
    if (Edges) return;
    for (int i=0; i<NbU*NbV; i+=NbV){
        if (Forme){
            glDrawArrays(GL_POLYGON, i, NbV);
        }else{
            glDrawArrays(GL_LINE_STRIP, i, NbV);
        }
    }

    for (int i=NbU*NbV; i<(NbU*NbV)*2; i+=NbU)
    {
        if (Forme) {
            glDrawArrays(GL_POLYGON, i, NbU);
        }else{
            glDrawArrays(GL_LINE_STRIP, i, NbU);
        }
    }
}
